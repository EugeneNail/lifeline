...
// Handler adapts the update-time-habit use case to the HTTP transport.
type Handler struct {
	usecase  *update_time_habit.Handler
	identity authentication.RequestIdentity
}

// NewHandler returns a transport handler wired to the update-time-habit use case.
func NewHandler(usecase *update_time_habit.Handler, identity authentication.RequestIdentity) *Handler {
	return &Handler{usecase: usecase, identity: identity}
}

// Payload represents the JSON request body for time habit updates.
type Payload struct {
...
func (handler *Handler) Handle(request *http.Request) (int, any) {
	...
	_, err = handler.usecase.Handle(request.Context(), update_time_habit.Command{
		ID:        habitID,
		Label:     payload.Label,
		Icon:      payload.Icon,
	...
	if err != nil {
		...
		return http.StatusInternalServerError, fmt.Errorf("handling UpdateTimeHabit command: %w", err)
	}
