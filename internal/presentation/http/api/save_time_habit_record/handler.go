...
import (
	...
	"github.com/EugeneNail/lifeline/internal/application/usecases/save_time_habit_record"
...
// Handler adapts the save-time-habit-record use case to the HTTP transport.
type Handler struct {
	usecase  *save_time_habit_record.Handler
	identity authentication.RequestIdentity
}

// NewHandler returns a transport handler wired to the save-time-habit-record use case.
func NewHandler(usecase *save_time_habit_record.Handler, identity authentication.RequestIdentity) *Handler {
	return &Handler{usecase: usecase, identity: identity}
}

// Payload represents the JSON request body for a time habit record save operation.
type Payload struct {
...
func (handler *Handler) Handle(request *http.Request) (int, any) {
	...
	if err := handler.usecase.Handle(request.Context(), save_time_habit_record.Command{
		...
		return http.StatusInternalServerError, fmt.Errorf("handling SaveTimeHabitRecord command: %w", err)
	}
