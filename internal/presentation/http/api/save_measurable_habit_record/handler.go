...
import (
	...
	"github.com/EugeneNail/lifeline/internal/application/usecases/save_measurable_habit_record"
...
// Handler adapts the save-measurable-habit-record use case to the HTTP transport.
type Handler struct {
	usecase  *save_measurable_habit_record.Handler
	identity authentication.RequestIdentity
}

// NewHandler returns a transport handler wired to the save-measurable-habit-record use case.
func NewHandler(usecase *save_measurable_habit_record.Handler, identity authentication.RequestIdentity) *Handler {
	return &Handler{usecase: usecase, identity: identity}
}

// Payload represents the JSON request body for a measurable habit record save operation.
type Payload struct {
...
func (handler *Handler) Handle(request *http.Request) (int, any) {
	...
	if err := handler.usecase.Handle(request.Context(), save_measurable_habit_record.Command{
		AccountId:         accountID.Uuid(),
		Value:             payload.Value,
		Date:              date,
		MeasurableHabitId: habitID,
	}); err != nil {
		...
		return http.StatusInternalServerError, fmt.Errorf("handling SaveMeasurableHabitRecord command: %w", err)
	}
